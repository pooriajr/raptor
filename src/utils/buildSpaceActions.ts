...
import { getSpaceByCoords, getSpaceOnTile, getTileById } from "./boardQueries";
import { hasActionPoints, isPhase, isSetupPhase } from "@/state/guards.ts";

type PieceType = "mother" | "baby" | "scientist";

type SpaceActionMap = Map<SpaceId, SpaceAction<GameAction>>;

type SetSpaceAction = (spaceId: SpaceId, style: SpaceStyle, action?: GameAction, tooltip?: string) => void;

type SpaceActionRule = {
  when: (state: GameState, selectedActorId: string | null) => boolean;
  run: (set: SetSpaceAction, state: GameState, selectedActorId: string | null) => void;
};

function createSpaceActionsBuilder(): { actions: SpaceActionMap; set: SetSpaceAction } {
  const actions: SpaceActionMap = new Map();
  const set: SetSpaceAction = (spaceId, style, action, tooltip) => {
    const existing = actions.get(spaceId);
    if (!existing) {
      actions.set(spaceId, { style, action, tooltip });
      return;
    }
...
function getActiveInteraction(state: GameState) {
  ...
  return { currentPlayer, interaction, selectedActorId: interaction.selectedActorId };
}
...
function getValidSetupSpaces(state: GameState, tileId: number): Array<{ x: number; y: number }> {
  ...
  for (const space of tile.spaces) {
    ...
    if (isPhase(state, "SCIENTIST_SETUP") && space.isExit) continue;
...
function collectPieceInfos(state: GameState): PieceInfo[] {
...
function getActionTargets(state: GameState, selectedActorId: string | null): ActionTargets {
  ...
  if (!isPhase(state, "ACTION_PHASE") || !selectedActorId || !hasActionPoints(state)) return result;
  ...
  const allPiecesInfo = collectPieceInfos(state);
...
function getValidMoves(
  ...
  if (!isPhase(state, "ACTION_PHASE") || !hasActionPoints(state)) return [];
  if (pieceInfo.type === "mother") {
    const woundCost = state.mother.paidWoundCost ? 0 : state.mother.sleepTokens;
    if (!options?.ignoreMotherWoundCost && !hasActionPoints(state, woundCost + 1)) return [];
  }
  ...
  return pieceInstance.getValidMoves(state.tiles, allPieces, state.fireTokens).filter((move) => {
    const targetSpace = getSpaceByCoords(state.tiles, move.tileId, move.x, move.y);
...
function addFireTokens(set: SetSpaceAction, state: GameState): void {
  for (const fire of state.fireTokens) {
    set(createSpaceId(fire.tileId, fire.x, fire.y), "fire");
  }
}

function addActionTargetsHighlights(set: SetSpaceAction, state: GameState, selectedActorId: string | null): void {
...
function addMothersCallHighlights(
  set: SetSpaceAction,
  state: GameState,
  selectedActorId: string | null,
  allPieces: ReturnType<typeof getAllBoardPositions>,
): void {
  if (!state.mother.position) return;
...
function addReinforcementsHighlights(set: SetSpaceAction, state: GameState): void {
  if (getReserveCount(state.scientists) <= 0) return;
  ...
  for (const tile of state.tiles) {
    ...
    for (let x = 0; x < 3; x++) {
      const space = getSpaceOnTile(tile, x, edgeY);
...
function addFirePlacementHighlights(set: SetSpaceAction, state: GameState): void {
  ...
  for (const key of allAdjacents) {
    ...
    if (!local) continue;
    const tile = getTileById(state.tiles, local.tileId);
    if (!tile) continue;
...
function addEffectPhaseHighlights(set: SetSpaceAction, state: GameState, selectedActorId: string | null): void {
  ...
  if (effectType === "mothers_call") {
    addMothersCallHighlights(set, state, selectedActorId, allPieces);
  }

  // Reinforcements: highlight valid edge spaces
  if (effectType === "reinforcements") {
    addReinforcementsHighlights(set, state);
  }
  ...
  if (effectType === "fire") {
    addFirePlacementHighlights(set, state);
  }

  // Jeep: two-step selection - first select scientist, then destination
  if (effectType === "jeep") {
    addJeepHighlights(set, state, selectedActorId, allPieces);
  }
}

function addRaptorActionSelection(set: SetSpaceAction, state: GameState, selectedActorId: string | null): void {
...
function addScientistActionSelection(set: SetSpaceAction, state: GameState, selectedActorId: string | null): void {
  for (const scientist of Object.values(state.scientists)) {
    if (!scientist.position) continue;

    // Frightened scientist: can stand up (costs 1 AP, not same round frightened)
    if (scientist.isFrightened) {
      if (hasActionPoints(state) && !scientist.frightenedThisRound) {
        const action: GameAction = { type: "ACTION_SCIENTIST_STAND_UP", scientistId: scientist.id };
        set(createSpaceId(scientist.position.tileId, scientist.position.x, scientist.position.y), "selectable", action);
      } else if (hasActionPoints(state) && scientist.frightenedThisRound) {
        set(
          createSpaceId(scientist.position.tileId, scientist.position.x, scientist.position.y),
          "disabled",
          undefined,
...
function addActionPhaseSelection(set: SetSpaceAction, state: GameState, selectedActorId: string | null): void {
  const player = state.activePlayer;

  if (player === "raptor") {
    addRaptorActionSelection(set, state, selectedActorId);
  } else if (player === "scientist") {
    addScientistActionSelection(set, state, selectedActorId);
  }
}

function addActionPhaseMoveTargets(set: SetSpaceAction, state: GameState, selectedActorId: string): void {
  const activePiece = getPieceInfo(state, selectedActorId);
  ...
  const cannotAffordMotherMove =
    activePiece.type === "mother" && hasActionPoints(state) && !hasActionPoints(state, woundCost + 1);
...
function addSetupRemovalTargets(set: SetSpaceAction, state: GameState): void {
  if (isPhase(state, "RAPTOR_SETUP")) {
    if (state.mother.position) {
...
function addSetupPlacementTargets(set: SetSpaceAction, state: GameState): void {
  const validTiles = getValidSetupTiles(state);
  for (const tileId of validTiles) {
    for (const { x, y } of getValidSetupSpaces(state, tileId)) {
      let action: GameAction | undefined;
      if (isPhase(state, "RAPTOR_SETUP")) {
        action = !isMotherPlaced(state) ? { type: "PLACE_MOTHER", tileId, x, y } : { type: "PLACE_BABY", tileId, x, y };
      } else if (isPhase(state, "SCIENTIST_SETUP")) {
...
function addSetupMoveTargets(set: SetSpaceAction, state: GameState): void {
...
function addSetupPhaseHighlights(set: SetSpaceAction, state: GameState): void {
  addSetupRemovalTargets(set, state);
  addSetupPlacementTargets(set, state);
  addSetupMoveTargets(set, state);
}

function addMotherReturnHighlights(set: SetSpaceAction, state: GameState): void {
  for (const tile of state.tiles) {
    for (const space of tile.spaces) {
...
const spaceActionRules: SpaceActionRule[] = [
  {
    when: (state, selectedActorId) => isPhase(state, "ACTION_PHASE") && !!selectedActorId && hasActionPoints(state),
    run: (set, state, selectedActorId) => addActionTargetsHighlights(set, state, selectedActorId),
  },
  {
    when: (state) => isPhase(state, "ACTION_PHASE"),
    run: (set, state, selectedActorId) => addActionPhaseSelection(set, state, selectedActorId),
  },
  {
    when: (state, selectedActorId) => isPhase(state, "ACTION_PHASE") && !!selectedActorId,
    run: (set, state, selectedActorId) => addActionPhaseMoveTargets(set, state, selectedActorId!),
  },
  {
    when: (state) => isPhase(state, "EFFECT_PHASE") && state.effectActionsRemaining > 0,
    run: (set, state, selectedActorId) => addEffectPhaseHighlights(set, state, selectedActorId),
  },
  {
    when: (state) => isSetupPhase(state),
    run: (set, state) => addSetupPhaseHighlights(set, state),
  },
  {
    when: (state) => isPhase(state, "MOTHER_RETURN"),
    run: (set, state) => addMotherReturnHighlights(set, state),
  },
];
...
export function buildSpaceActions(state: GameState): SpaceActions<GameAction> {
  const { actions, set } = createSpaceActionsBuilder();
  const { selectedActorId } = getActiveInteraction(state);

  addFireTokens(set, state);

  for (const rule of spaceActionRules) {
    if (rule.when(state, selectedActorId)) {
      rule.run(set, state, selectedActorId);
    }
  }
